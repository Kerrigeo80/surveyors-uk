...
- **Survey report delivery** (migration `job_report_delivery`). Closes the core gap — the surveyor's deliverable. `job_reports` table + private `reports` storage bucket (RLS: won surveyor uploads; org+surveyor+admin read). On a won job (ReportSection in RequestDetailModal): **"Create with Quotem ↗"** = external link DEAD-END stub (real integration deferred — needs Quotem API/partnership) + **upload report+photos+notes** (`submitReport`). Org/admin download (DocumentLink got a `bucket` prop), then mark complete. Flow: post → win → survey → **deliver report → org reviews → complete & pay**.
- **LinkedIn importer + invite links.** Importer now accepts RAW LinkedIn exports (First Name+Last Name→name, Email Address, Profile URL, Current Title, Current Company, Location→region). Saves to `linkedin_profiles` on the Import button. **Invite links** (migration `linkedin_invite_lookup`): admin LinkedIn pool → "Copy link" → `/register?invite=<id>` → Register prefills via anon RPC `get_linkedin_invite` + auto-merges on signup (`claim_linkedin_profile`). 25 surveyors imported. Kerri outreaches via LinkedIn Recruiter (exports have no emails). FUTURE: "Send invite" email via Resend once profiles have emails (+ PECR/GDPR caveat).
- **2nd admin:** Deana Gaines, marie-88@hotmail.co.uk (role admin, via `seed_auth_user`; sets password via Forgot Password).
- Security advisors clean — remaining warnings all intentional (pg_net in public; anon `submit_hazard_report`/`org_public_name`/`get_linkedin_invite`; authenticated `claim_linkedin_profile`/`accept_offer`).
